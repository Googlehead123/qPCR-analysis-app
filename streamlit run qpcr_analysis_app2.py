...
# Define patterns for required columns. Note the inclusion of 'Cт' (Cyrillic T)
COLUMN_PATTERNS: Dict[str, List[str]] = {
    'sample': ['sample name', 'sample', 'samplename', 'name'],
    'target': ['target name', 'target', 'targetname', 'gene', 'assay', 'reporter'],
    'ct': ['ct', 'cт', 'cq', 'c t', 'c т', 'value', 'ct value']
}
...
# ------------------------------------------------------------
# ROBUST DATA SCANNING FUNCTIONS
# ------------------------------------------------------------
...
def scan_for_columns_and_start(df: pd.DataFrame, file_name: str) -> Tuple[Optional[Dict[str, str]], int]:
    ...
    for i in range(min(100, len(df_rows))):
        
        # Assume the current row 'i' is the header
        header_row = [str(x).lower().strip().replace('_', ' ') for x in df_rows[i]]
        
        found_columns: Dict[str, str] = {}
        
        # Look for required patterns in this row
        for required_key, patterns in COLUMN_PATTERNS.items():
            for col_index, cell_value in enumerate(header_row):
                # Clean cell value for robust matching
                col_clean = re.sub(r'[^a-z0-9]', '', cell_value)
                
                for pattern in patterns:
                    pattern_clean = re.sub(r'[^a-z0-9]', '', pattern)
                    
                    if pattern_clean == col_clean or pattern_clean in col_clean:
                        # Store the *original* column title found at this index
                        found_columns[required_key] = df.columns[col_index]
                        break # Move to the next required key
    ...
    # Fallback: if no clear header is found, return None
    return None, -1
...
def scan_excel_file(uploaded_file: io.BytesIO, ct_cutoff: float) -> pd.DataFrame:
    """
    Reads an Excel file, finds the header row, and extracts required data.
    """
    ...
    try:
        ...
        if file_name.lower().endswith('.csv'):
            # CSV files often only need one read
            sheets = {'Sheet1': pd.read_csv(uploaded_file, header=None, engine='python', encoding='utf-8', sep=None, on_bad_lines='skip', skipinitialspace=True)}
        else:
            # Excel files need to be read without an initial header to scan all rows
        ...
        for sheet_name, df_raw in sheets.items():
            ...
            if found_cols_map is None:
                continue # Skip this sheet if data header wasn't found
            
            # --- Re-read/Slice DataFrame using the detected header ---
            # Use the detected row index as the new header
            header_row = df_raw.iloc[header_row_index]
            df_data = df_raw[header_row_index+1:].copy()
            df_data.columns = header_row # Set the header titles as column names
            df_data = df_data.reset_index(drop=True)
            
            # Map the original titles to the standardized ones
            sample_col_title = found_cols_map['sample']
            target_col_title = found_cols_map['target']
            ct_col_title = found_cols_map['ct']

            # --- Extract and Standardize Data ---
            extracted_data = pd.DataFrame()
            
            # 1. Sample Name (String, clean whitespace)
            extracted_data['Sample Name'] = df_data[sample_col_title].astype(str).str.strip()
            
            # 2. Target Name (String, uppercase and clean whitespace)
            extracted_data['Target Name'] = df_data[target_col_title].astype(str).str.strip().str.upper()
            
            # 3. Ct (Numeric, apply parsing and cutoff)
            extracted_data['Ct'] = df_data[ct_col_title].apply(lambda x: parse_ct_value(x, cutoff=ct_cutoff))
        ...
        st.error(f"Error reading/parsing **{file_name}** on sheet **{sheet_name}**. Please check file format and columns: {str(e)}")
        return pd.DataFrame()
...
# ------------------------------------------------------------
# CALCULATION ENGINE (Identical to previous, focusing on raw ΔCt for stats)
# ------------------------------------------------------------
...
def create_bar_graph(data: pd.DataFrame, gene: str, log_scale: bool) -> Optional[plt.Figure]:
    ...
    # Ensure reference samples are first, then other samples
    ...
    # Calculate the max height for marker positioning
    ...
    for i, (exp, err, sig) in enumerate(zip(expression, errors, significance)):
        if sig in ['*', '**', '***']:
            # Position marker slightly above the error bar (5% of max height)
    ...
    **Flexible qPCR data analyzer** - Upload single or multiple files. The app automatically finds the data table,
            ...
            with st.spinner(f"Scanning {len(uploaded_files)} file(s) for data start rows..."):
                ...
                for file in uploaded_files:
                    ...
                    if not scanned.empty:
                        ...
                        st.warning(f"⚠️ No valid data found in **{file.name}** after applying Ct cutoff.")
    ...
    if 'scanned_data' in st.session_state and st.session_state['scanned_data'] is not None:
        ...
        with col1:
            ...
            # Default to first available gene if not set
        ...
        with col2:
            ...
            # Default to previously selected samples if available
    ...
    if st.session_state.get('analysis_run', False):
        ...
        with col1:
            ...
            output = io.BytesIO()
            # Ensure the Ct data types are consistent for Excel export
